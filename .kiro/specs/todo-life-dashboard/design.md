...
- **Module pattern**: Each widget is encapsulated in its own JS object to avoid global state pollution and make unit testing straightforward.
...
sequenceDiagram
    ...
    StorageService-->>app.js: AppState with defaults
...
StorageService = {
  ...
  get(key),       // Returns parsed value or null on failure
  set(key, value), // Returns true on success, false on quota/error
  loadAll(),      // Returns AppState with defaults for missing/corrupt keys
}
`
...
GreetingWidget = {
  ...
  getGreetingPhrase(hour), // pure function -> greeting string
  formatTime(date),        // pure function -> "HH:MM:SS"
  formatDate(date),        // pure function -> "Day, DD Month YYYY"
}
`

`getGreetingPhrase`, `formatTime`, and `formatDate` are pure functions with no side effects, making them straightforward to property-test.
...
Internal state: `remainingSeconds`, `intervalId`, `isRunning`, `pomoDuration`.
...
ThemeManager = {
  init(savedTheme),
  toggle(),
  apply(theme),             // sets data-theme on body element
  current(),
}
`
...
*For any* integer hour in the range 0-23, `GreetingWidget.getGreetingPhrase(hour)` SHALL return exactly one of "Good Morning", "Good Afternoon", "Good Evening", or "Good Night" - with no hour left unclassified and no hour mapping to more than one phrase.

**Validates: Requirements 2.1, 2.2, 2.3, 2.4**

### Property 4: User name trimming and validation

*For any* string input to the name field, the system SHALL trim leading and trailing whitespace before validation. If the trimmed result is between 1 and 50 characters, it SHALL be accepted and saved. If the trimmed result is empty (whitespace-only input), the previous name SHALL be retained unchanged.
...
*For any* integer input, `FocusTimerWidget.validateDuration(input)` SHALL return the integer if it is between 1 and 60 inclusive, and SHALL return null for any value outside that range (including non-integers, negative numbers, and zero).
...
*For any* task, toggling its completion status twice SHALL return the task to its original completion state. That is, `toggle(toggle(task)).completed === task.completed` for all tasks.

**Validates: Requirements 4.4, 4.5**

### Property 10: Task persistence round-trip

*For any* array of Task objects, serializing the array to localStorage via `StorageService.set` and then reading it back via `StorageService.get` SHALL produce an array that is deeply equal to the original (same ids, titles, and completion statuses).
...
*For any* label and URL pair, `QuickLinksWidget.validateLink(label, url)` SHALL return invalid if the label is empty, the URL is empty, the label exceeds 100 characters, or the URL exceeds 2048 characters; and SHALL return valid for any pair where both label and URL are non-empty and within their respective length limits.
...
- The calling widget checks the return value and, if `false`, displays a non-blocking warning banner (a dismissible toast at the top of the page).
...
- Tag: `Feature: todo-life-dashboard, Property 4: User name trimming and validation`

**Property 5: Timer display format is always MM:SS**
- Generator: integers 0-3600
- Assertion: output matches `/^\d{2}:\d{2}$/` with valid ranges
- Tag: `Feature: todo-life-dashboard, Property 5: Timer display format is always MM:SS`

**Property 6: Duration validation accepts only 1-60**
- Generator: arbitrary integers (including out-of-range)
- Assertion: returns integer for 1-60, null otherwise
- Tag: `Feature: todo-life-dashboard, Property 6: Duration validation accepts only 1-60`
...
- Tag: `Feature: todo-life-dashboard, Property 7: Adding a valid task grows the list by one`

**Property 8: Invalid task titles are rejected**
- Generator: strings composed only of whitespace characters
- Assertion: validateTitle returns false, list unchanged
- Tag: `Feature: todo-life-dashboard, Property 8: Invalid task titles are rejected`

**Property 9: Toggle completion is an involution**
- Generator: arbitrary Task objects
- Assertion: double-toggle returns original completed state
- Tag: `Feature: todo-life-dashboard, Property 9: Toggle completion is an involution`

**Property 10: Task persistence round-trip**
- Generator: arbitrary Task arrays
- Assertion: serialize then deserialize produces deeply equal array
- Tag: `Feature: todo-life-dashboard, Property 10: Task persistence round-trip`

**Property 11: URL normalization always produces http/https prefix**
- Generator: arbitrary strings
- Assertion: result starts with http:// or https://; already-prefixed inputs unchanged
- Tag: `Feature: todo-life-dashboard, Property 11: URL normalization always produces http/https prefix`
...
- Tag: `Feature: todo-life-dashboard, Property 12: Link validation rejects out-of-bounds inputs`
